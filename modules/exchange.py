...
class Exchange:
    ...
    def process_swapzone_exchange(self, address, amount):
        self.logger.info("Processing Exchange")
        amount = self.truncate((amount / self.config['atomic']),4)
        self.logger.info(f"Exchange Amount: {amount}")
        url = 'https://4sp4w95r1i.execute-api.us-west-2.amazonaws.com/default/exchange'
        data_in = {"from": self.config['convert_from'],
                   "to": self.config['convert_to'],
                   "addressReceive": self.config['address_to'],
